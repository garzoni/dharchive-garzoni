...
JSONEditor.AbstractEditor = Class.extend({
  ...
  getTitle: function() {
	...
	if(this.schema.label){
		if(typeof this.schema.label === "object"){
			if(this.schema.label.hasOwnProperty(JSONEditor.defaults.default_language)){
				t = this.schema.label[JSONEditor.defaults.default_language];
			}else if(this.schema.label.hasOwnProperty(JSONEditor.defaults.fallback_language)){
				t = this.schema.label[JSONEditor.defaults.fallback_language];
			}else{
				t = this.key;
			}
		}else{
	...
	if(this.schema.required && this.schema.required == true)
	t = t + " *";
	return t;
  },
  enable: function() {
...
JSONEditor.defaults.editors.object = JSONEditor.AbstractEditor.extend({
  ...
  layoutEditors: function() {
    ...
    // Normal layout
      ...
      $each(this.property_order, function(i,key) {
        ...
        if(editor.options.hidden) row.style.display = 'none';
  ...
  build: function() {
    ...
    // If the object should be rendered as a div
      ...
      this.editjson_textarea.style.width = '100px';
...
JSONEditor.defaults.editors.multiple = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    if(window.jQuery && window.jQuery.fn && window.jQuery.fn.dropdown) {
		window.jQuery(this.switcher).dropdown();
    }

    this.editor_holder = document.createElement('div');
    this.editor_holder.style.marginTop = "10px";
...
JSONEditor.defaults.editors.select = JSONEditor.AbstractEditor.extend({
  setValue: function(value,initial) {
    var sanitized;
    if(this.schema && this.schema._choiceList){
    	...
    	if(value !== "" && value !== null){
    		...
    		window.jQuery.ajax(
    			{
    				url : e.jsoneditor.options.choiceList.url + "?listQName=" + e.schema._choiceList.listQName + "&key="+value+"&labelProperty="+e.schema._choiceList.labelProperty+"&keyProperty="+e.schema._choiceList.keyProperty+"&language="+JSONEditor.defaults.default_language,
    	...
    	for(var z = 0; z < value.length; z++){
    		if( inp.find('option[value="'+value+'"]').length==0)
    			inp.append('<option value="'+value+'">'+dropdownValue+'</option>');
    	}
    ...
    if(this.select2) this.select2.select2('val',this.input.value);
   	if(this.dropdown){
    	...
    	setTimeout(function(){
    		that.dropdown.dropdown('set exactly', val);
    	}, 1);
  ...
  preBuild: function() {
    ...
    }else if(this.schema._choiceList){

		this.schema._choiceList.labelProperty = this.schema._choiceList.labelProperty || "label";
		this.schema._choiceList.keyProperty = this.schema._choiceList.keyProperty || "id";
		this.schema._choiceList.minCharacters = this.schema._choiceList.minCharacters || 0;
		this.enum_options = [];
		this.enum_display = [];
    }
    // Other, not supported
  ...
  onInputChange: function() {
    ...
    var new_val;
    if(this.schema._choiceList){

		new_val = this.dropdown.dropdown('get value');
    }else{
  ...
  setupDropdown: function() {
    // If the Semantic ui dropdown library is loaded use it when we have lots of items
    if(window.jQuery && window.jQuery.fn && window.jQuery.fn.dropdown ) {
      var options = $extend({},JSONEditor.plugins.dropdown);
      if(this.schema.options && this.schema.options.dropdown_options) options = $extend(options,this.schema.options.dropdown_options);
      window.jQuery(this.input).addClass('search');
      if(this.schema._choiceList){
		options.apiSettings = {
			url : this.jsoneditor.options.choiceList.url + "?listQName=" + this.schema._choiceList.listQName + "&labelProperty=" + this.schema._choiceList.labelProperty+"&keyProperty="+this.schema._choiceList.keyProperty+"&language="+JSONEditor.defaults.default_language+"&label={query}"
		};
	  	options.minCharacters = this.schema._choiceList.minCharacters;
	  }
	  //options.debug = true;
      options.forceSelection = false;
	  options.saveRemoteData = false;
	  options.showOnFocus = options.minCharacters > 0 ? false : true;
      var self = this;
      options.onChange = function(value, text, choice) {
        var dropdown = window.jQuery(this).closest('.ui.dropdown'),
        ...
        if (value) {
          ...
          icon.on('click', function(event) {
            ...
            window.jQuery(this).removeClass('delete');
            window.jQuery(this).addClass('dropdown');
      ...
      this.dropdown = window.jQuery(this.input).dropdown(options);
  ...
  enable: function() {
    if(!this.always_disabled) {
		...
		if(this.dropdown) this.dropdown.removeClass("disable");
    }
    this._super();
  },
  disable: function() {
    ...
    if(this.dropdown) this.dropdown.addClass("disabled");
    this._super();
  },
  destroy: function() {
    if(this.label && this.label.parentNode) this.label.parentNode.removeChild(this.label);
...
JSONEditor.defaults.editors.multiselect = JSONEditor.AbstractEditor.extend({
  preBuild: function() {
	...
	if(this.schema.items && this.schema.items._choiceList){
			this.schema.items._choiceList.labelProperty = this.schema.items._choiceList.labelProperty || "label";
			this.schema.items._choiceList.keyProperty = this.schema.items._choiceList.keyProperty || "id";
			this.schema.items._choiceList.minCharacters = this.schema.items._choiceList.minCharacters || 0;
	}
  ...
  build: function() {
	...
	}else if((!this.schema.format && this.option_keys.length < 8) || this.schema.format === "checkbox") {
  ...
  setValue: function(value, initial) {
    ...
    if(this.schema.items._choiceList){
    	var vals = [];
    	for(var k in value){
    		e = this;
    		window.jQuery.ajax(
    			{
    				url : e.jsoneditor.options.choiceList.url + "?listQName=" + e.schema.items._choiceList.listQName + "&key="+value[k]+"&labelProperty="+e.schema.items._choiceList.labelProperty+"&keyProperty="+this.schema.items._choiceList.keyProperty+"&language="+JSONEditor.defaults.default_language,
					success: function(result){
        				e.dropdownValueList.push(result);
    				},
    				async: false
    			}
    		);
  ...
  setupDropdown: function() {
    if(window.jQuery && window.jQuery.fn && window.jQuery.fn.dropdown) {
        var options = window.jQuery.extend({},JSONEditor.plugins.dropdown);
        if(this.schema.options && this.schema.options.dropdown_options) options = $extend(options,this.schema.options.dropdown_ptions);
		window.jQuery(this.input).addClass('search');

        if(this.schema.items && this.schema.items._choiceList){
			options.apiSettings = {
				url : this.jsoneditor.options.choiceList.url + "?listQName=" + this.schema.items._choiceList.listQName + "&labelProperty="+this.schema.items._choiceList.labelProperty+"&keyProperty="+this.schema.items._choiceList.keyProperty+"&language="+JSONEditor.defaults.default_language+"&label={query}"
			};
	     	options.minCharacters = this.schema.items._choiceList.minCharacters;
        }
        options.forceSelection = false;
        options.saveRemoteData = false;
	  	options.showOnFocus = options.minCharacters > 0 ? false : true;
		//options.debug = true;
		var self = this;
		options.onRemove = options.onChange = function(value, text, choice) {
			self.value = self.dropdown.dropdown('get value');
			self.onChange(true);
		};
		this.dropdown = window.jQuery(this.input).dropdown(options);
  ...
  updateValue: function(value) {
    ...
    if(this.schema.items && this.schema.items._choiceList){
    	var inp = $(this.input);
    	for(var z = 0; z < value.length; z++){
    		if( inp.find('option[value="'+value[z]+'"]').length==0)
    			inp.append('<option value="'+value[z]+'">'+this.dropdownValueList[z]+'</option>');
    	}
    	this.value = value;
    	changed = true;
    }else{
	...
	if(this.dropdown){
    	...
    	setTimeout(function(){
    		that.dropdown.dropdown('set exactly', vals);
    	}, 1);
  ...
  enable: function() {
    if(!this.always_disabled) {
      ...
      if(this.select2) this.select2.select2("enable",true);
      if(this.dropdown) this.dropdown.removeClass('disabled');
    }
    this._super();
  },
  disable: function() {
...
JSONEditor.defaults.editors.checkbox = JSONEditor.AbstractEditor.extend({
  ...
  setupCheckbox: function() {
    ...
    // If the Semantic ui checkbox library is loaded use it when we have lots of items
    if(window.jQuery && window.jQuery.fn && window.jQuery.fn.checkbox ) {

      var options = $extend({},JSONEditor.plugins.checkbox);
      if(this.schema.options && this.schema.options.checkbox_options) options = $extend(options,this.schema.options.checkbox_options);

      this.checkbox = window.jQuery(this.input).checkbox(options);
    }
    else {
      this.checkbox = null;
    }
...
JSONEditor.defaults.themes.semantic = JSONEditor.AbstractTheme.extend({

  getButton: function(text, icon, title) {
    var el = this._super(text, icon, title);
    el.className += 'ui button tiny';
    return el;
  },
  getFormInputLabel: function(text) {
    var el = this._super(text);
  ...
  addInputError: function(input, text) {
    ...
    var group = this.closest(input,'.form-control');
    group.className = group.className.replace( /(?:^|\s)error(?!\S)/g , '' );
  ...
  getFormControl: function(label, input, description) {
    var el = document.createElement('div');
    el.className = 'form-control field';
    var wrap;
    if(input.type === 'checkbox') {

      wrap = document.createElement('div');
  	  wrap.setAttribute('class', 'ui checkbox');
  	  wrap.appendChild(input);
      if(label) wrap.appendChild(label);
  	  else wrap.appendChild(document.createElement('label'));
  	  el.appendChild(wrap);
    }
    	...
    	wrap = document.createElement('div');
    	wrap.setAttribute('class', 'ui input');
    	wrap.appendChild(input);
    	el.appendChild(wrap);
    }else{
      if(label) el.appendChild(label);
...
JSONEditor.defaults.iconlibs.semantic = JSONEditor.AbstractIconLib.extend({
  mapping: {
    collapse: 'chevron down',
    expand: 'chevron right',
    "delete": 'remove',
    ...
    cancel: 'undo',
    save: 'save',
    moveup: 'arrow circle outline up',
    movedown: 'arrow circle outline down'
  },
  icon_prefix: 'ui icon '
});
...
JSONEditor.defaults.theme = 'semantic';
...
JSONEditor.defaults.fallback_language = 'fr';
...
// Use the `select` editor for dynamic enumSource enums
JSONEditor.defaults.resolvers.unshift(function(schema) {
  if(schema._choiceList) return 'select';
});
...
JSONEditor.defaults.resolvers.unshift(function(schema) {
  ...
  if(schema.type === "array" && schema.items && !(Array.isArray(schema.items)) && schema.uniqueItems && ['string','number','integer'].indexOf(schema.items.type) >= 0) {
    ...
    if(schema.items.enum || schema.items._choiceList) {
      return 'multiselect';
    }
